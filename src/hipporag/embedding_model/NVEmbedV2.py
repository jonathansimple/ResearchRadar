...
class NVEmbedV2EmbeddingModel(BaseEmbeddingModel):
    ...
    def batch_encode(self, texts: List[str], **kwargs) -> None:
        ...
        print("Embedding batch size + max_length: ", self.global_config.embedding_batch_size, self.global_config.embedding_max_seq_len)
        original_max_len = self.global_config.embedding_max_seq_len
        ...
        batch_size = params.pop("batch_size", 2)
        ...
        if len(texts) <= batch_size:
            ...
            for i in range(0, len(texts), batch_size):
                ...
                params["max_length"] = original_max_len
                try:
                    results.append(self.embedding_model.encode(**params))
                except:
                    # consider passing
                    print("CUDA OOM ON ENTRIES:", params["prompts"])
                    
                    try:
                        params["max_length"] = original_max_len/2
                        results.append(self.embedding_model.encode(**params))
                    except:
                        print("halving max_len to", params["max_length"], " didn't work - passing entry")
                        pass
