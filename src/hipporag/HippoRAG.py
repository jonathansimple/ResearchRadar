...
class HippoRAG:
    ...
    def __init__(self,
        ...
        self.working_dir = os.path.join(self.global_config.save_dir, f"{llm_label.replace(':','')}_{embedding_label}")
        ...
        self.openie_results_path = os.path.join(self.global_config.save_dir,f'openie_results_ner_{self.global_config.llm_name.replace("/", "_").replace(":","")}.json')
    ...
    def index(self, docs: List[str]):
        ...
        #print("triple_results_dict", triple_results_dict)

        #print("len(chunk_to_rows), len(ner_results_dict), len(triple_results_dict)")
        #print(len(chunk_to_rows), len(ner_results_dict), len(triple_results_dict))
        ...
        print("entity_nodes insertion done")

        logger.info(f"Encoding Facts")
        print("facts")
        #print(facts)
        self.fact_embedding_store.insert_strings([str(fact) for fact in facts])
        print("facts insertion done")

        logger.info(f"Constructing Graph")

        self.node_to_node_stats = {}
        self.ent_node_to_chunk_ids = {}
        print("add_fact_edges")
        self.add_fact_edges(chunk_ids, chunk_triples)
        print("add_fact_edges done")
        print("add_passage_edges")
        num_new_chunks = self.add_passage_edges(chunk_ids, chunk_triple_entities)
        print("add_passage_edges done")
    ...
    def retrieve(self,
        ...
        for q_idx, query in tqdm(enumerate(queries), desc="Retrieving", total=len(queries)):
            ...
            if len(top_k_facts) == 0:
                logger.info('Hippo retrieval: No facts found after reranking, return DPR results')
        ...
        if gold_answers is not None:
            ...
            for idx, q in enumerate(queries_solutions):
                if not isinstance(gold_answers, list):
                    print("processing gold_answers, which is currently of type:", type(gold_answers))
                    print(gold_answers)
                    gold_answers = [gold_answers]  # wrap it in a list
                if isinstance(gold_answers[idx], float):
                    print("processing gold_answers[idx], which is currently of type:", type(gold_answers[idx]))
                    print(gold_answers[idx])
                    gold_answers[idx] = str(gold_answers[idx])
    ...
    def get_top_k_weights(self,
        ...
        #print(np.count_nonzero(all_phrase_weights))
        #print(len(linking_score_map.keys()))
